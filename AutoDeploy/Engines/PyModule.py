from BaseEngine import BaseEngine, registerEngineFactory
...
def PyModule(module_name):
    ...
    def __pymodule_installer(self, sudo = False):
        if self.VirtualEnv is not None:
            command("%s/bin/pip install %s" % (self.VirtualEnv, self.module_name))
        else:        
    ...
    def requestVariable(self, string):
        if(string == "VirtualEnv"): return self.VirtualEnv.app_path
